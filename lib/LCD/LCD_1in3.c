...
void LCD_1IN3_DisplayWindows(uint16_t Xstart, uint16_t Ystart, uint16_t Xend, uint16_t Yend, uint16_t *Image)
{
    ...
    dma_channel_configure(dma_tx,
                          &c,
                          &spi_get_hw(LCD_SPI_PORT)->dr, //this is different to the source
                          Image, // read address
                          160*144*2,//you need x2 because RGB565 colour data is 16 bits (the 12bit colour encoding seems cumbersome) //This argument is the size of the transmission data
                          true);
   dma_finish = false;
