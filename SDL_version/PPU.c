...
static const uint32_t DMGcolours[4] = {
    0xFFFFFFFF, // white
...
void ppu(struct cartridge *cart ,cpu *CPU, memory *mem, ppu_data *data){
    ...
    if((LCDC(mem)&0x80) == 0){//LCD off
        ...
        uint8_t scanline_dots = CPU->frame_timer % 456;
        uint8_t mode;
        if(LY(mem) >= 144){//so I only have to do this once
            //mode 1 Vblank
            // puts("mode 1");
            // fflush(stdout);
            ...
            } else if((scanline_dots >= 80) && (!data->finish || scanline_dots > 200)){
                if(scanline_dots == 80){
                    // puts("mode 3");
                    // fflush(stdout);
                    memset(data->BGcolours,0,8);
